...
<body>
	<main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
		<div id="Top-navbar" class="flex items-center justify-between w-full px-5 pt-5">
			<a href="{{ url()->previous() }}">
		...
		<section id="brand"
          ...
          {{ $brand->products->count() }} Products</p>
			</div>
		</section>
						...
						<div class="w-20 h-20 flex shrink-0 rounded-2xl overflow-hidden bg-[#F6F6F6] items-center">
							<div class="w-full h-[50px] flex shrink-0 justify-center">
								<img src="{{ Storage::url($product->thumbnail) }}"
									class="object-contain w-full h-full" alt="thumbnail" />
							</div>
						</div>
        ...
        @empty
          <p>Belum ada produk dari {{ $brand->name }} dan kategori ini</p>
        @endforelse
